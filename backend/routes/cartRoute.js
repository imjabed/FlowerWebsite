...
Router.post('/custom',verifyToken, async(req,res)=>{
    const userId = req.user.id;
    const { flowerCount, flowerColor, wrapperColor, selectedDecorations, makingCostFree, totalPrice } = req.body;
    try{
        let cart = await Cart.findOne({userId});
        if(!cart) { cart = new Cart({userId, customItems: [] }) }
    ...
    {
        console.error(err);
        res.status(500).json({ message: 'Failed to add custom bouquet' });
    }
})
...
Router.delete('/custom/:customId', verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.status(500).json({ message: 'Failed to remove custom bouquet' });
  }
});
